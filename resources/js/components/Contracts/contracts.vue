        ...
        <div style="text-align: left">
            <div class="row">
                ...
                <div class="col-1 v-question-block">
                    <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="currentColor" class="bi bi-question-circle v-question" viewBox="0 0 40 40">
                        <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
                        <path d="M5.255 5.786a.237.237 0 0 0 .241.247h.825c.138 0 .248-.113.266-.25.09-.656.54-1.134 1.342-1.134.686 0 1.314.343 1.314 1.168 0 .635-.374.927-.965 1.371-.673.489-1.206 1.06-1.168 1.987l.003.217a.25.25 0 0 0 .25.246h.811a.25.25 0 0 0 .25-.25v-.105c0-.718.273-.927 1.01-1.486.609-.463 1.244-.977 1.244-2.056 0-1.511-1.276-2.241-2.673-2.241-1.267 0-2.655.59-2.75 2.286zm1.557 5.763c0 .533.425.927 1.01.927.609 0 1.028-.394 1.028-.927 0-.552-.42-.94-1.029-.94-.584 0-1.009.388-1.009.94z"/>
                    </svg>
                    <p class="v-question-p">Для фильтрации данных по ключевому запросу введи текст в поле. Таблица будет перестроена под запрос автоматически.</p>
                </div>
            </div>
        </div>
        <table-lite
    ...
    setup(props) {
        ...
        let findReports = function (company_id) {
            ...
            let filteredReports = reports.filter(report=>{
                return projectsOfCompanyIds.includes(report.contract_id);
            })
            let values = {
                quarter_1: 0,
                quarter_2: 0,
        ...
        const table = reactive({
            ...
            columns: [
                ...
                },
                {
                    ...
                    sortable: true,
                    display: function (row) {
                        return (`<a href="../../contract/info/${row.id}" target="_blank" class="btn btn-success">Карточка контракта</a>`);
                    },
                },
            ],
            rows: [],
        ...
        const doSearch = async (reload) => {
            ...
            if (reload) {
                ...
                let reportUrl;
                if (props.user_id == 1) {
                    reportUrl = "../../contract/getReportAll";
                } else {
                    reportUrl = `../../contract/getReportByUser/${props.user_id}`;
                }
                ...
                let uId = props.user_id,
                    url = uId === 1 ? "../contract/getContractAll" : `../contract/getContractByUser/${props.user_id}`;
                await axios.get(url)
                    .then(response => {
                        table.rows = [];
