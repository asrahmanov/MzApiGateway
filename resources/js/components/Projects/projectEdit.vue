    ...
    <div>
        ...
        <div v-else>
            ...
            <div class="row" >
                <div class="col-6">
                    <div class="form-group">
                        <label>Прогнозируемая выручка, МЛН.РУБ. </label>
                ...
                <div class="col-12" style="display: none">
                    <div class="form-group">
                        ...
                        <select class="form-control" v-model="type_id" v-else>
                            <option value="0">Выберите Статус проекта</option>
                            <option :value="item.id" v-for="item in projectType">{{ item.name }}</option>
                        </select>
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-6">
                    <div class="form-group">
                        ...
                        <select name="project_type" class="form-control" v-model="status_id" v-if="!editField" disabled>
                            <option value="0">Выберите статус проекта</option>
                            <option :value="item.id" v-for="item in projectStatus">{{ item.name }}</option>
                        </select>

                        <select name="project_type" id="type_id" class="form-control" v-model="status_id" v-else>
                            <option value="0">Выберите статус проекта</option>
                            <option :value="item.id" v-for="item in projectStatus">{{ item.name }}</option>
                        </select>
                    </div>
                ...
                <div class="col-6">
                    <div class="form-group">
                        <label>Статус этап проекта</label>
                        <select name="project_type" class="form-control" v-model="stage_id" v-if="!editField" disabled>
                            <option value="0">Выберите этап проекта</option>
                            <option :value="item.id" v-for="item in projectStage">{{ item.name }}</option>
                        </select>

                        <select name="project_type" class="form-control" v-model="stage_id" v-else>
                            <option value="0">Выберите этап проекта</option>
                            <option :value="item.id" v-for="item in projectStage">{{ item.name }}</option>
                        </select>
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control" v-model="appointment_id" v-if="!editField" disabled>
                    <option value="0">Выберите тип направления проекта</option>
                    <option :value="item.id" v-for="item in projectAppointment">{{ item.name }}</option>
                </select>

                <select class="form-control" v-model="appointment_id" v-else>
                    <option value="0">Выберите тип направления проекта</option>
                    <option :value="item.id" v-for="item in projectAppointment">{{ item.name }}</option>
                </select>
...
export default {
    ...
    data() {
        return {
            ...
            projectStatus: [], // Массив статусов проекта
            status_id: 0, // Статус проекта

            projectStage: [], // Массив этапов прокта
            ...
            company: '', // Выбранная компания для кого создается проект наименование
    ...
    methods: {
            ...
            axios
                ...
                .then((response) => {
                    for (let key in response.data) {
                        this.projectAppointment.push(response.data[key]);
                    }
                })
        },
        ...
        loadProjectStatus() {
            axios
                .get(`../../project/getStatus`)
                .then((response) => {
                    for (let key in response.data) {
                        this.projectStatus.push(response.data[key]);
                    }
                })
        },
        ...
        loadProjectStage() {
            axios
                ...
                .then((response) => {
                    for (let key in response.data) {
                        this.projectStage.push(response.data[key]);
                    }
                })
        },
        ...
        loadProjectType() {
            axios
                ...
                .then((response) => {
                    for (let key in response.data) {
                        this.projectType.push(response.data[key]);
                    }
                })
        },
        ...
        save() {
            ...
            if (this.price == 0) {
                ...
                axios.post(`../../project/update`, data)
                    .then(response => {
                        if (response.data) {
                            this.loading = false
